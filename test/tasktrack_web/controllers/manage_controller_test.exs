defmodule TasktrackWeb.ManageControllerTest do
  use TasktrackWeb.ConnCase

  alias Tasktrack.Assignments
  alias Tasktrack.Assignments.Manage

  @create_attrs %{managee_id: "some managee_"}
  @update_attrs %{managee_id: "some updated managee_"}
  @invalid_attrs %{managee_id: nil}

  def fixture(:manage) do
    {:ok, manage} = Assignments.create_manage(@create_attrs)
    manage
  end
  ...
  describe "update manage" do
    ...
    test "renders manage when data is valid", %{conn: conn, manage: %Manage{id: id} = manage} do
      ...
      assert json_response(conn, 200)["data"] == %{
        "id" => id,
        "managee_id" => "some updated managee_id"}
    end
